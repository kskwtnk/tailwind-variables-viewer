...
- [x] **ネームスペース検出**: 各プレフィックスが正しく分類される
- [x] **カスタムネームスペース**: 未知のプレフィックスは`other`に分類
- [x] **タイプ判定**: color/size/font/referenceの正確な判定
- [x] **短縮名抽出**: プレフィックス除去が正しい
- [x] **ソート**: spacingが数値順、他はアルファベット順
- [x] **重複処理**: 同名変数の上書き
- [x] **空の入力**: エラーなく空オブジェクトを返す

### 検証方法

Scenario 1-4のテスト実行により、以下が確認済み:
- 375個のデフォルト変数が正しくネームスペース別に分類
- color/size/fontのタイプ判定が正確
- 短縮名（例: `mint-500`）が正しく抽出
- フロントエンドで適切にソート表示
...
- [x] **デフォルトポート**: 3000で起動
- [x] **カスタムポート**: 指定ポートで起動
- [x] **ポート自動検索**: 使用中の場合に次のポート検索
- [x] **API正常応答**: `/api/variables`がJSONを返す
- [x] **静的ファイル**: フロントエンド配信
- [x] **404エラー**: 存在しないパスで404
- [x] **graceful shutdown**: SIGINT処理
