...
async function testScenario(cssFile: string) {
	...
	if (parsedTheme.hasImport) {
		try {
			...
			console.warn(
				"Warning: tailwindcss not found, skipping default variables",
			);
		}
	}
...
describe("Integration Tests - Scenarios", () => {
	test('Scenario 1: Default only (@import "tailwindcss")', async () => {
		const result = await testScenario(
			join(SCENARIOS_DIR, "1-default-only.css"),
		);
	...
	test("Scenario 3: Extend defaults (default + custom)", async () => {
		const result = await testScenario(
			join(SCENARIOS_DIR, "3-extend-defaults.css"),
		);
	...
	test("Scenario 4: Reset and custom only", async () => {
		const result = await testScenario(
			join(SCENARIOS_DIR, "4-reset-and-custom.css"),
		);
		...
		// 特定の変数を確認
		const primaryVar = result.organized.color?.find(
			(v) => v.varName === "--color-primary",
		);
